...
**Neue Kampagnenmissionen gelten erst mit fertiger Vertonung als abgeschlossen:** Vor einem Browserstimmen-Fallback immer zuerst GPU (`nvidia-smi`) sowie Qwen3-TTS und Speaches außerhalb einer gegebenenfalls blockierenden Sandbox prüfen. Sind die lokalen Dienste verfügbar, müssen neue Briefings und Erfolgsberichte im selben Auftrag mit `scripts/generate_audio.py` erzeugt, auf −16 LUFS gemastert, per Speaches verifiziert und anschließend als `narration: recorded` mit neuer `audioRevision` eingebunden werden. `narration: browser` ist nur zulässig, wenn die lokalen Dienste nach tatsächlicher Prüfung nicht verfügbar sind oder der Auftrag ausdrücklich auf Audioerzeugung verzichtet; diese Einschränkung bei der Übergabe offen nennen. Eine verfügbare GPU nicht bloß aufgrund fehlgeschlagenen Sandbox-Zugriffs als unavailable behandeln.
