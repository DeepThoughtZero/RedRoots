...
for(const file of ['js/utils/Constants.js','js/core/AI.js','js/core/Grid.js','js/core/Territory.js','js/campaign/Missions.js','js/campaign/Story.js','js/campaign/Act2.js','js/campaign/ObjectiveSystem.js','js/campaign/CampaignManager.js','js/core/GameState.js'])vm.runInContext(fs.readFileSync(file,'utf8'),ctx);
...
function place(s,key,r,c){assert.equal(s.placePattern(C.PATTERNS[key].pattern,r,c),true,`mission ${s.scenario.id}: ${key} at ${r},${c}`);}
