...
for(const f of ['utils/Constants','core/Grid','core/Territory','core/AI','campaign/Missions','campaign/Story','campaign/Act2','campaign/Act3','campaign/Act4','campaign/ObjectiveSystem','campaign/CampaignManager','core/GameState'])vm.runInContext(fs.readFileSync('js/'+f+'.js','utf8'),ctx);
...
const legacyDigits='3333333333';let p=new CampaignState();assert.equal(p.importCode(`RR2-${legacyDigits}-${CampaignState.checksum(legacyDigits)}`),true);assert.equal(p.available(10),true);assert.equal(p.available(11),false);p.record('A3_M01',2);const code=p.exportCode();assert.match(code,/^RR4-[0-3]{20}-/);storage.clear();p=new CampaignState();assert.equal(p.importCode(code),true);assert.equal(p.completed.A3_M01.stars,2);assert.equal(p.completed.A2_M05.stars,3);assert.equal(p.importCode(`RR1-33333-${CampaignState.checksum('33333')}`),true);assert.equal(p.completed.A3_M01.stars,2);
...
console.log('PASS: five Act III solutions, extinction/order/quarantine failures, dual-front defeat, continuous holding, RR1/RR2/RR3 compatibility and checkpoints');
