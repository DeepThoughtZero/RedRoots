...
for (const file of ['js/utils/Constants.js','js/core/Grid.js','js/core/Territory.js','js/campaign/Missions.js','js/campaign/Story.js','js/campaign/Act2.js','js/campaign/ObjectiveSystem.js','js/campaign/CampaignManager.js','js/core/GameState.js']) vm.runInContext(readFileSync(file,'utf8'),context);
...
function place(s,key,r,c) { assert.equal(s.placePattern(C.PATTERNS[key].pattern,r,c),true,`${key} can be placed at ${r},${c}`); }
...
(async () => {
    ...
    let progress=new CampaignState(); assert.equal(progress.genomes.join(','),'cell'); assert.equal(progress.available(1),false); progress.record(missions[0].id,3); progress.record(missions[0].id,1); progress=new CampaignState(); assert.equal(progress.completed.A1_M01.stars,3); assert.equal(progress.available(1),true); assert.ok(progress.genomes.includes('block')); assert.equal(progress.genomes.includes('blinker'),false); progress.record(missions[1].id,1); assert.ok(progress.genomes.includes('blinker')); assert.ok(progress.genomes.includes('glider'));
