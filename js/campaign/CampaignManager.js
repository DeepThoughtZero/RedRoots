class CampaignState {
    ...
    exportCode() {
        ...
        return `RR3-${digits}-${CampaignState.checksum(digits)}`;
    }
    static get passwords() { return ['PALISADE-LANDUNG', 'PALISADE-WURZEL', 'PALISADE-PASS', 'PALISADE-WASSER', 'PALISADE-GRENZE', 'PALISADE-WASSERSTROM', 'PALISADE-SCHLEUSEN', 'PALISADE-FAEHRE', 'PALISADE-ARCHIVE', 'PALISADE-INSELN', 'PALISADE-SCHLUESSEL', 'PALISADE-STILLE', 'PALISADE-RUECKWEG', 'PALISADE-SIEGEL', 'PALISADE-NETZ']; }
    importCode(input) {
        ...
        if (checkpoint >= 0) {
            ...
            const match = legacy || /^RR2-([0-3]{10})-([0-9A-Z]{2})$/.exec(code) || /^RR3-([0-3]{15})-([0-9A-Z]{2})$/.exec(code);
        ...
        this.overlay.innerHTML = `
            ...
            ${actComplete ? `<section class="campaign-completion"><h2>Akt ${chapter} abgeschlossen</h2><p>${nextAct ? `Die Reise geht weiter: Akt ${nextAct.roman} · ${nextAct.name} ist jetzt spielbar.` : 'Die Zentrale ist abgeschaltet, die Siedlungen leben. Alle fünfzehn aktuellen Missionen sind geschafft. Der Sturm im Norden führt die Geschichte in einem späteren Akt fort.'}</p>${nextAct ? `<button class="launch-button" data-act="${nextAct.id}">Weiter zu Akt ${nextAct.roman} →</button>` : ''}</section>` : ''}<div class="expedition-layout"><div class="mars-chart"><div class="chart-caption">MARS / EXPEDITIONSROUTE </div>
                <div class="mars-globe"><div class="mars-surface"></div><div class="mars-grid"></div>
                    ...
                    <span class="planet-label label-tharsis">THARSIS<small>VULKANPLATEAU</small></span><span class="planet-label label-chryse">${m.act === 1 ? 'CHRYSE · LANDEEBENE' : m.act === 2 ? 'HELLAS · WASSERSIEDLUNGEN' : 'OLYMPUS · SCHUTZANLAGE'}</span><span class="planet-label label-valles">${m.act === 1 ? 'VALLES · SCHLUCHTEN' : m.act === 2 ? 'SCHUTZGÜRTEL' : 'UNTER DEM VULKAN'}</span>
